...
export default function Command() {
  ...
  const nowPlayingDetails = useMemo(() => {
    ...
    if (state.current.type === "track") {
      return {
        subtitle: [state.current.grandparentTitle, state.current.parentTitle]
          .filter(Boolean)
          .join(" - "),
      };
    }
  ...
  const upNextItems =
    state.queue && currentQueueIndex >= 0
      ? state.queue.items.slice(currentQueueIndex + 1)
      : (state.queue?.items ?? []);
  const backToItems =
    state.queue && currentQueueIndex > 0
      ? [...state.queue.items.slice(0, currentQueueIndex)].reverse()
      : [];
  const currentTrack =
  ...
  function renderQueueItems(items: PlayQueueInfo["items"]) {
    return items.map((track) => {
      ...
      const index = queueItems.findIndex(
        (item) => item.playQueueItemID === track.playQueueItemID,
      );
          ...
          actions={
            <ActionPanel>
              <Action
                ...
                onAction={() =>
                  runControl(
                    () => skipToQueueItem(track),
                    "Skipped to selected track",
                  )
                }
              />
              <Action
                ...
                icon={Icon.Music}
                onAction={() =>
                  void runNavigation(() => navigateToAlbum(track))
                }
              />
              <Action
                title="Go to Artist"
                icon={Icon.Person}
                onAction={() =>
                  void runNavigation(() => navigateToArtist(track))
                }
              />
  ...
  return (
    <List
      isLoading={isLoading || isPerforming}
      searchBarPlaceholder="Inspect the current Plexamp queue"
    >
          ...
          actions={
            <ActionPanel>
              <Action
                ...
                icon={Icon.Play}
                onAction={() =>
                  runControl(() => playPause(), "Playback toggled")
                }
              />
              {currentTrack ? (
                <Action
                  ...
                  icon={Icon.Music}
                  onAction={() =>
                    void runNavigation(() => navigateToAlbum(currentTrack))
                  }
                />
              ) : null}
              <Action
              ...
              {currentTrack ? (
                <Action
                  ...
                  icon={Icon.Person}
                  onAction={() =>
                    void runNavigation(() => navigateToArtist(currentTrack))
                  }
                />
              ) : null}
              <Action
                ...
                icon={Icon.Forward}
                onAction={() =>
                  runControl(() => skipNext(), "Skipped to next track")
                }
              />
              <Action
                ...
                icon={Icon.Rewind}
                onAction={() =>
                  runControl(() => skipPrevious(), "Returned to previous track")
                }
              />
              <Action
                title="Stop"
                icon={Icon.Stop}
                onAction={() => runControl(() => stop(), "Playback stopped")}
              />
              {renderClearQueueAction()}
              <LibraryActions />
              <PreferencesAction />
        ...
        <>
          <List.Section title="Up Next">
            {upNextItems.length > 0 ? (
              renderQueueItems(upNextItems)
            ) : (
              <List.Item title="No upcoming tracks" />
            )}
          </List.Section>
          <List.Section title="Back To">
            {backToItems.length > 0 ? (
              renderQueueItems(backToItems)
            ) : (
              <List.Item title="No previous tracks" />
            )}
          </List.Section>
        </>
